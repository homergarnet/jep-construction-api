    ...
    public class ReviewController : ControllerBase
    {
        ...
        )
        {
            ...
            try
            {
                ...
                var getReviewList = _iReviewService.GetReviewList(keyword ?? "", userId, isApprove, page, pageSize);
        ...
        [Authorize]
        [HttpPut]
        [Route("update-approve-review-post")]
        public async Task<IActionResult> UpdateApproveReviewPost([FromBody] UpdateApproveReviewPostRequest req)
        {
            try
            {
                var userEmailAdd = User.FindFirst("UserEmailAdd")?.Value;
                var result = _iReviewService.UpdateApproveReviewPost(req);
                if (!result.IsSuccess && result.ApiMessage.Equals(ReviewConstants.UPDATE_APPROVE_REVIEW_FAILED))
                {
                    return new ContentResult
                    {
                        StatusCode = 400,
