namespace jep_construction_api.Constants
{
    public static class ClientRequestConstants
    {
        public const string CREATE_CLIENT_REQUEST_SUCCESS = "Create client request successfully created";
        public const string INVALID_EMAIL_ADDRESS = "Invalid email address";
        public const string INVALID_CLIENT_REQUEST_ID = "Invalid client request id";
        public const string CLIENT_REQUEST_NOT_FOUND = "Client request not found";
        public const string SOFT_DELETE_CLIENT_REQUEST_SUCCESS = "SoftDeleteClientRequestById successfully executed";
        public const string SOFT_DELETE_CLIENT_REQUEST_FAILED = "SoftDeleteClientRequestById no update changes";


    }
}
